...
// ── MoonTone microphone uplink: Opus encoder JNI ────────────────────────────
// Single-stream Opus encoder used by the upcoming bidirectional microphone path.

JNIEXPORT jlong JNICALL
Java_com_limelight_nvstream_jni_MoonBridge_opusEncoderCreate(JNIEnv *env, jclass clazz,
...
JNIEXPORT jint JNICALL
Java_com_limelight_nvstream_jni_MoonBridge_opusEncoderEncode(JNIEnv *env, jclass clazz,
                                                             jlong encoder,
                                                             jshortArray pcm,
                                                             jint frameSize,
                                                             jbyteArray out) {
    OpusEncoder *enc = (OpusEncoder *) (intptr_t) encoder;
    if (enc == NULL) {
        return -1;
    }
    ...
    int len = opus_encode(enc, (const opus_int16 *) pcmBuf, frameSize,
...
JNIEXPORT void JNICALL
Java_com_limelight_nvstream_jni_MoonBridge_opusEncoderDestroy(JNIEnv *env, jclass clazz,
                                                              jlong encoder) {
    OpusEncoder *enc = (OpusEncoder *) (intptr_t) encoder;
    if (enc != NULL) {
