...
static PSDP_OPTION getAttributesList(char*urlSafeAddr) {
    ...
    if (IS_SUNSHINE()) {
        ...
        // MoonTone extension: tell Sunshine this is an audio-only session
        if (StreamConfig.audioOnly) {
            err |= addAttributeString(&optionHead, "x-ml-audio-only", "1");
        }
...
static int fillSdpTail(char* buffer, size_t length) {
    if (StreamConfig.audioOnly) {
        // No video m-line for audio-only sessions
