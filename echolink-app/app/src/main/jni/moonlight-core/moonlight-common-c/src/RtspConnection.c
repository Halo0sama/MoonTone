    ...
    *error = -1;
    ...
    ret = initializeRtspRequest(&request, "ANNOUNCE",
                                StreamConfig.audioOnly
                                    ? (APP_VERSION_AT_LEAST(7, 1, 431) ? controlStreamId : "streamid=audio")
...
int performRtspHandshake(PSERVER_INFORMATION serverInfo) {
    ...
    // Audio-only sessions skip the video SETUP entirely.
    if (!StreamConfig.audioOnly) {
        RTSP_MESSAGE response;
        int error = -1;
        char* pingPayload;
    ...
    if (APP_VERSION_AT_LEAST(7, 1, 431)) {
        ...
        // Audio-only sessions skip the legacy video PLAY.
        if (!StreamConfig.audioOnly) {
            RTSP_MESSAGE response;
            int error = -1;
